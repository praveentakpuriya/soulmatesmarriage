<div class="hero-wrap"
        style="background-image: url('assets/img/bg_3.jpg');height: 300px;background-size: cover;background-position: bottom;">

        <div class="container">
            <div class="row no-gutters slider-text justify-content-start align-items-center">
                <div class="col-lg-6 col-md-6  d-flex align-items-end">
                    <div class="text">
                        <h1 class="mb-4">Membership<span></span> <span></span></h1>
                    </div>
                </div>

            </div>
        </div>
    </div>


<section class="mt-5">
    <div class="container">
        <div class="row">
        <div class="col-md-12">
            <div class="sectiondeib">
                <!-- <h4>Membership Benefits</h4> -->
            <p><i class="fa fa-check-square-o" aria-hidden="true"></i> Dedicated Relationship Manager</p>
            <p><i class="fa fa-check-square-o" aria-hidden="true"></i> Discreet and Confidential service</p>
            <p><i class="fa fa-check-square-o" aria-hidden="true"></i>
     Relationship Manager confirms the prospect interest to save your valuable time✔️(only with Platinum and Premium Dimond)</p>
            <p> <i class="fa fa-check-square-o" aria-hidden="true"></i>
 A senior Relationship manager with extensive experience will look for the perfect match for you✔️(only with Platinum and Premium Dimond)</p>
        </div>
        </div>
        </div>
    </div>
</section>

<section class="membaer">
        <div class="pricing pricing-palden">
 

            <div class="pricing-item features-item ja-animate" data-animation="move-from-bottom" data-delay="item-0" style="min-height: 497px;">
                <div class="pricing-deco">
                    <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" y="0px">
                        <path class="deco-layer deco-layer--1" d="M30.913,43.944c0,0,42.911-34.464,87.51-14.191c77.31,35.14,113.304-1.952,146.638-4.729c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--2" d="M-35.667,44.628c0,0,42.91-34.463,87.51-14.191c77.31,35.141,113.304-1.952,146.639-4.729c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--3" d="M43.415,98.342c0,0,48.283-68.927,109.133-68.927c65.886,0,97.983,67.914,97.983,67.914v3.716H42.401L43.415,98.342z" fill="#FFFFFF" opacity="0.7"></path>
                        <path class="deco-layer deco-layer--4" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z" fill="#FFFFFF"></path>
                    </svg>
                    <div class="pricing-price"><span class="pricing-currency"></span>4000/-
                        <!-- <span class="pricing-period">/ day</span> -->
                    </div>
                    <h3 class="pricing-title">CLASSIC</h3>
                </div>
                <ul class="pricing-feature-list">
                    <li class="pricing-feature">Membership Duration 90 Days</li>
                    <li class="pricing-feature">  Send Message.</li>
                    <li class="pricing-feature">Edit your profile.</li>
                    <li class="pricing-feature">Upload your photo.</li>
                    <li class="pricing-feature">Express Interest - send "Express</li>
                    <li class="pricing-feature">Interest" messages to anyone you like.</li>
                </ul>
                <button class="pricing-action">Upgrade Now</button>
            </div>
            <div class="pricing-item features-item ja-animate pricing__item--featured" data-animation="move-from-bottom" data-delay="item-1" style="min-height: 497px;">
                <div class="pricing-deco" style="background: linear-gradient(135deg,#a93bfe,#584efd)">
                    <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" y="0px">
                        <path class="deco-layer deco-layer--1" d="M30.913,43.944c0,0,42.911-34.464,87.51-14.191c77.31,35.14,113.304-1.952,146.638-4.729c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--2" d="M-35.667,44.628c0,0,42.91-34.463,87.51-14.191c77.31,35.141,113.304-1.952,146.639-4.729c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--3" d="M43.415,98.342c0,0,48.283-68.927,109.133-68.927c65.886,0,97.983,67.914,97.983,67.914v3.716H42.401L43.415,98.342z" fill="#FFFFFF" opacity="0.7"></path>
                        <path class="deco-layer deco-layer--4" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z" fill="#FFFFFF"></path>
                    </svg>
                    <div class="pricing-price"><span class="pricing-currency"></span>8000/-
                        <!-- <span class="pricing-period">/ month</span> -->
                    </div>
                    <h3 class="pricing-title">CLASSIC GOLD</h3>
                </div>
                <ul class="pricing-feature-list">
                    <li class="pricing-feature">Membership Duration 120 Days</li>
                     <li class="pricing-feature">Total Contact View 95</li>
                    <li class="pricing-feature">  Send Message.</li>
                    <li class="pricing-feature">Edit your profile.</li>
                    <li class="pricing-feature">Upload your photo.</li>
                    <li class="pricing-feature">Express Interest - send "Express</li>
                    <li class="pricing-feature">Interest" messages to anyone you like.</li>
                </ul>
                <button class="pricing-action">Upgrade Now</button>
            </div>
            <div class="pricing-item features-item ja-animate" data-animation="move-from-bottom" data-delay="item-2" style="min-height: 497px;">
                <div class="pricing-deco">
                    <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" y="0px">
                        <path class="deco-layer deco-layer--1" d="M30.913,43.944c0,0,42.911-34.464,87.51-14.191c77.31,35.14,113.304-1.952,146.638-4.729c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--2" d="M-35.667,44.628c0,0,42.91-34.463,87.51-14.191c77.31,35.141,113.304-1.952,146.639-4.729c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--3" d="M43.415,98.342c0,0,48.283-68.927,109.133-68.927c65.886,0,97.983,67.914,97.983,67.914v3.716H42.401L43.415,98.342z" fill="#FFFFFF" opacity="0.7"></path>
                        <path class="deco-layer deco-layer--4" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z" fill="#FFFFFF"></path>
                    </svg>
                    <div class="pricing-price"><span class="pricing-currency"></span>1100/-
                        <!-- <span class="pricing-period">/ week</span> -->
                    </div>
                    <h3 class="pricing-title">CLASSIC PLUS</h3>
                </div>
                <ul class="pricing-feature-list">
                    <li class="pricing-feature">Membership Duration 180 Days</li>
                    <li class="pricing-feature">Total Contact View 110</li>
                    <li class="pricing-feature">  Send Message.</li>
                    <li class="pricing-feature">Edit your profile.</li>
                    <li class="pricing-feature">Upload your photo.</li>
                    <li class="pricing-feature">Express Interest - send "Express</li>
                    <li class="pricing-feature">Interest" messages to anyone you like.</li>
                </ul>
                <button class="pricing-action">Upgrade Now</button>
            </div>
       
</div>  

   <div class="pricing pricing-palden mt-5 mb-4">
            <div class="pricing-item features-item ja-animate" data-animation="move-from-bottom" data-delay="item-0" style="min-height: 497px;">
                <div class="pricing-deco">
                    <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" y="0px">
                        <path class="deco-layer deco-layer--1" d="M30.913,43.944c0,0,42.911-34.464,87.51-14.191c77.31,35.14,113.304-1.952,146.638-4.729c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--2" d="M-35.667,44.628c0,0,42.91-34.463,87.51-14.191c77.31,35.141,113.304-1.952,146.639-4.729c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--3" d="M43.415,98.342c0,0,48.283-68.927,109.133-68.927c65.886,0,97.983,67.914,97.983,67.914v3.716H42.401L43.415,98.342z" fill="#FFFFFF" opacity="0.7"></path>
                        <path class="deco-layer deco-layer--4" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z" fill="#FFFFFF"></path>
                    </svg>
                    <div class="pricing-price"><span class="pricing-currency"></span>15000/-
                        <!-- <span class="pricing-period">/ day</span> -->
                    </div>
                    <h3 class="pricing-title">CLASSIC DIAMOND</h3>
                </div>
                <ul class="pricing-feature-list">
                    <li class="pricing-feature">Membership Duration 240 Days</li>
                    <li class="pricing-feature">Total Contact View 150</li>
                    <li class="pricing-feature">  Send Message.</li>
                    <li class="pricing-feature">Edit your profile.</li>
                    <li class="pricing-feature">Upload your photo.</li>
                    <li class="pricing-feature">Express Interest - send "Express</li>
                    <li class="pricing-feature">Interest" messages to anyone you like.</li>
                </ul>
                <button class="pricing-action">Upgrade Now</button>
            </div>
            <div class="pricing-item features-item ja-animate pricing__item--featured" data-animation="move-from-bottom" data-delay="item-1" style="min-height: 497px;">
                <div class="pricing-deco" style="background: linear-gradient(135deg,#a93bfe,#584efd)">
                    <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" y="0px">
                        <path class="deco-layer deco-layer--1" d="M30.913,43.944c0,0,42.911-34.464,87.51-14.191c77.31,35.14,113.304-1.952,146.638-4.729c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--2" d="M-35.667,44.628c0,0,42.91-34.463,87.51-14.191c77.31,35.141,113.304-1.952,146.639-4.729c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--3" d="M43.415,98.342c0,0,48.283-68.927,109.133-68.927c65.886,0,97.983,67.914,97.983,67.914v3.716H42.401L43.415,98.342z" fill="#FFFFFF" opacity="0.7"></path>
                        <path class="deco-layer deco-layer--4" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z" fill="#FFFFFF"></path>
                    </svg>
                    <div class="pricing-price"><span class="pricing-currency"></span>20000/-
                        <!-- <span class="pricing-period">/ month</span> -->
                    </div>
                    <h3 class="pricing-title">PLATINUM PLAN</h3>
                </div>
                <ul class="pricing-feature-list">
                    <li class="pricing-feature">Membership Duration 270 Days</li>
                     <li class="pricing-feature">Total Contact View 200</li>
                    <li class="pricing-feature">  Send Message.</li>
                    <li class="pricing-feature">Edit your profile.</li>
                    <li class="pricing-feature">Upload your photo.</li>
                    <li class="pricing-feature">Express Interest - send "Express</li>
                    <li class="pricing-feature">Interest" messages to anyone you like.</li>
                </ul>
                <button class="pricing-action">Upgrade Now</button>
            </div>
            <div class="pricing-item features-item ja-animate" data-animation="move-from-bottom" data-delay="item-2" style="min-height: 497px;">
                <div class="pricing-deco">
                    <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" y="0px">
                        <path class="deco-layer deco-layer--1" d="M30.913,43.944c0,0,42.911-34.464,87.51-14.191c77.31,35.14,113.304-1.952,146.638-4.729c48.654-4.056,69.94,16.218,69.94,16.218v54.396H30.913V43.944z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--2" d="M-35.667,44.628c0,0,42.91-34.463,87.51-14.191c77.31,35.141,113.304-1.952,146.639-4.729c48.653-4.055,69.939,16.218,69.939,16.218v54.396H-35.667V44.628z" fill="#FFFFFF" opacity="0.6"></path>
                        <path class="deco-layer deco-layer--3" d="M43.415,98.342c0,0,48.283-68.927,109.133-68.927c65.886,0,97.983,67.914,97.983,67.914v3.716H42.401L43.415,98.342z" fill="#FFFFFF" opacity="0.7"></path>
                        <path class="deco-layer deco-layer--4" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z" fill="#FFFFFF"></path>
                    </svg>
                    <div class="pricing-price"><span class="pricing-currency"></span>25000/-
                        <!-- <span class="pricing-period">/ week</span> -->
                    </div>
                    <h3 class="pricing-title">CLASSIC PLATINUM</h3>
                </div>
                <ul class="pricing-feature-list">
                    <li class="pricing-feature">Membership Duration 365 Days</li>
                    <li class="pricing-feature">Total Contact View 300</li>
                    <li class="pricing-feature">  Send Message.</li>
                    <li class="pricing-feature">Edit your profile.</li>
                    <li class="pricing-feature">Upload your photo.</li>
                    <li class="pricing-feature">Express Interest - send "Express</li>
                    <li class="pricing-feature">Interest" messages to anyone you like.</li>
                </ul>
                <button class="pricing-action">Upgrade Now</button>
            </div>
       
</div>

    </section>